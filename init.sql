-- Create sequences and tables for mye_r database
...
-- Table: public.watchlistitem (Create this FIRST)
...
(
    ...
    status character varying(20) COLLATE pg_catalog."default" DEFAULT 'new',
    ...
    CONSTRAINT status_check CHECK (status IN ('new', 'indexed', 'indexing_failed', 'library_matched', 'matcher_failed', 'ready_for_download', 'scrape_failed', 'downloaded', 'symlink_failed', 'completed')),
    CONSTRAINT current_step_check CHECK (current_step IN ('new', 'indexing', 'indexed', 'librarymatching', 'library_matched', 'scraping', 'scraped', 'downloading', 'downloaded', 'symlinking', 'symlinked', 'completed', 'indexing_pending', 'librarymatch_pending', 'matching', 'scraping_pending', 'download_pending', 'symlink_pending'))
)
...
IS E'Overall status of the watchlist item';

-- Table: public.seasons (Now this can reference watchlistitem)
...
(
    ...
    CONSTRAINT tv_episodes_season_id_fkey FOREIGN KEY (season_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE CASCADE,
    CONSTRAINT fk_scrape_result FOREIGN KEY (scrape_result_id)
        REFERENCES public.scrape_results (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE SET NULL
)
TABLESPACE pg_default;
...
-- Create indexes (These can be after tables)
...
CREATE INDEX IF NOT EXISTS idx_scrape_results_downloaded
    ON public.scrape_results USING btree
    (downloaded ASC NULLS LAST)
    TABLESPACE pg_default;