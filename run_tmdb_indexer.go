...
func main() {
	...
	for _, itemID := range itemIDs {
		...
		if err != nil {
			...
			// Update item status to indexing_failed if not already updated
			if item.Status.String != "indexing_failed" {
				...
				if updateErr := db.UpdateWatchlistItem(item); updateErr != nil {
					myLogger.Error("main", "Process", fmt.Sprintf("Failed to update item status: %v", updateErr))
				}
			}
			continue
		}

		// Update item status to indexed and set next step
		updatedItem.Status = sql.NullString{String: "indexed", Valid: true}
		updatedItem.CurrentStep = sql.NullString{String: "librarymatch_pending", Valid: true}
		if err := db.UpdateWatchlistItem(updatedItem); err != nil {
			myLogger.Error("main", "Process", fmt.Sprintf("Failed to update item status: %v", err))
			continue
		}
