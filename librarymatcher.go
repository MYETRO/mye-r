...
func (lm *LibraryMatcher) ProcessNextItem() {
	...
	if len(matchedLibraries) > 0 {
		...
		item.CurrentStep = sql.NullString{String: "library_matched", Valid: true}
		...
		item.CurrentStep = sql.NullString{String: "library_matched", Valid: true}
		lm.log.Info("LibraryMatcher", "ProcessNextItem", "No custom library match found for item, matched to default library, proceeding to next step")
	}
...
func (lm *LibraryMatcher) checkFilter(item *database.WatchlistItem, filter config.Filter) bool {
	...
	case "media_type":
		if item.MediaType.Valid && item.MediaType.String == "tv" {
			match := strings.EqualFold(filter.Value, "tv")
			if match {
				lm.log.Debug("LibraryMatcher", "checkFilter", fmt.Sprintf("Media type match: %s against %s", item.MediaType.String, filter.Value))
			}
			return match
		}
	default:
	...
	return false // Ensure a return value is provided if no conditions are met
}
...
func (lm *LibraryMatcher) ProcessItemByID(itemID int) {
	...
	if len(matchedLibraries) > 0 {
		...
		item.CurrentStep = sql.NullString{String: "library_matching", Valid: true}
